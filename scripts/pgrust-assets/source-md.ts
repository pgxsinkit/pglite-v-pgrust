/**
 * `src/vendor/pgrust/SOURCE.md` — the provenance file, which after the release path records two
 * different things that must not be conflated:
 *
 * 1. the **vendored host JS**, copied byte-verbatim from a pgrust checkout and committed here, and
 * 2. the **binary assets** in `public/pgrust/`, which are gitignored and now normally arrive from a
 *    GitHub Release rather than from that checkout.
 *
 * They can be from different pgrust commits — a release download does not touch the vendored JS —
 * so the assets half is a marked block that each sync mode rewrites without disturbing the other,
 * and a commit mismatch between the two is reported loudly rather than papered over.
 *
...
export function readVendorCommit(markdown: string): string | null {
  const beforeAssets = markdown.split(ASSETS_BEGIN)[0] ?? "";
  const match = /^- Commit: `([0-9a-f]{7,40})`/m.exec(beforeAssets);
  return match?.[1] ?? null;
}
...
export function extractAssetsSection(markdown: string): string | null {
  return ASSETS_BLOCK.exec(markdown)?.[1] ?? null;
}

/**
 * Put `section` in the assets block, replacing any existing one; null removes the block.
 *
 * The rest of the file is left byte-for-byte alone, which is what lets `--release` update the
 * assets provenance without claiming anything new about the vendored JS.
 */
export function replaceAssetsSection(markdown: string, section: string | null): string {
  const withoutBlock = markdown.replace(ASSETS_BLOCK, "\n");
  if (section === null) {
